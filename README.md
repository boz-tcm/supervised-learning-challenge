...
- ## Summary of the Analysis
    Resampling our original dataset through random oversampling on the minority class label - loans at high risk of default - which was the primary target label that were trying to predict, materially improved Model 2's prediction accuracy and performance compared to Model 1.  Random oversampling improved representation of high risk loans in our resampled dataset and led to better prediction performance in Model 2, where the balanced accuray score increased to 99.6% from Model 1's 94.8%, where Model 1 likely suffered from sub-optimal training on the original, imbalanced peer-to-peer lending training dataset, and reflected in sub-par Model 1 testing predictions compared to Model 2.  Furthermore, had we relied on the traditional accuracy score, which does not account for the effect of bias in imbalanced training datasets, we may have wrongly concluded that Model 1's predictive ability is comparable to Model 2's as reflected in their nearly identical traditional accuracy scores of 99.3% and 99.5%, respectively.  However, deterioration in Model 1's balanced accuracy score to 94.8% revealed the inherent weakness in Model 1's true predictive power, whereas Model 2's balanced accuracy score (99.6%) was consistent with its traditional accuracy score (99.5%), reflecting Model 2's training on a resampled and balanced dataset.

    We also reviewed F1 scores from the models' classification reports to drill down into the models' predictive performance by class label.  The F1 Score is the harmonic mean of two important class prediction performance measures, Precision and Recall, and therefore takes into account both false positives and false negatives in a single composite measure. Furthermore, according to [statology](https://www.statology.org/f1-score-vs-accuracy), "we often use accuracy when the classes are balanced and there is no major downside to predicting false negatives [and we] often use F1 score when the classes are imbalanced and there is a serious downside to predicting false negatives."  Given loans at high risk of default is the positive label that we are primarily interested in modeling and predicting, reflecting the material economic consequences of correctly predicting this label, there is a serious downside in our case to predicting false negatives, or classifying loans at high risk of default as healthy.  Examining the classification reports for high-risk loans reveals that Model 2's F1 Score of 0.93, or 93%, was materially improved versus Model 1's F1 Score of 0.89, or 89%.  Moreover, drilling further into the Precision and Recall components of the F1 Score, while both models produced comparable Precision scores of 0.87, the Recall Score improved remarkably from 0.90, or 90%, for Model 1 to 1.00, or 100% (rounded up in classification report from TPR of 99.7%), for Model 2.  The upshot is that while Precision takes into account false positives, Recall takes into account false negatives, which as noted above is most important to the nature of our problem, and therefore Model 2's 100% recall, or true-positive rate (99.7% to three decimal places) commends Model 2 as the superior model for our peer-to-peer lending platform.
...
Room for improvement: _Investigate deriving or identifying explanatory power by independent variable.
...
<img src="Resources/Images/joshua-hoehne-Nsaqv7v2V7Q-unsplash.jpg" align="left"/>
<img src="Resources/Images/evan-wise-1SLDCdUZAcM-unsplash.jpg" align="right"/>